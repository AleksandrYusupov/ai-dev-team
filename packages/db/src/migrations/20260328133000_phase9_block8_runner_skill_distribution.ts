...
const runnerInventoryViewSql = `
  create view runner_inventory_view as
    select
      ...
      cap.active_agent_library_release_id,
      cap.active_agent_library_fingerprint,
      coalesce(cap.skill_sync_status, 'degraded') as skill_sync_status,
      cap.skill_sync_error,
      coalesce(cap.installed_skill_bundles, '[]'::jsonb) as installed_skill_bundles,
...
export async function up(db: Kysely<Database>): Promise<void> {
  await sql.raw(`
    alter table runner_capabilities
      add column active_agent_library_release_id text null,
      add column active_agent_library_fingerprint text null,
      add column skill_sync_status text not null default 'degraded',
      add column skill_sync_error text null,
      add column installed_skill_bundles jsonb not null default '[]'::jsonb;
...
export async function down(db: Kysely<Database>): Promise<void> {
  await sql.raw(`
    ...
    alter table runner_capabilities
      drop column if exists installed_skill_bundles,
      drop column if exists skill_sync_error,
      drop column if exists skill_sync_status,
      drop column if exists active_agent_library_fingerprint,
      drop column if exists active_agent_library_release_id;
  `).execute(db)
}
