...
const previousProviderFailoverMetricsViewSql = `
  create view provider_failover_metrics_view as
    with fallback_reason_counts as (
      select coalesce(
        jsonb_object_agg(fallback_reason, reason_count),
...
export async function up(db: Kysely<Database>): Promise<void> {
  await sql.raw(`
    alter table runner_nodes
      add column latest_mcp_pool_snapshot_json jsonb null,
      add column latest_mcp_pool_snapshot_at timestamptz null;
...
export async function down(db: Kysely<Database>): Promise<void> {
  await sql.raw(`
    ...
    ${previousProviderFailoverMetricsViewSql}

    alter table runner_nodes
      drop column if exists latest_mcp_pool_snapshot_at,
      drop column if exists latest_mcp_pool_snapshot_json;
  `).execute(db)
}
